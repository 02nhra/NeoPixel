...
NeoPixel::NeoPixel(uint16_t n, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 3), pin(p), bright(NULL), pixels(NULL)
...

{
  ...
  if((bright = (uint8_t *)malloc(numLEDs))) {
    memset(bright, 0, numLEDs);
  }
}
...
void NeoPixel::setBrightnessPixel(uint8_t b, uint8_t o) {//o = pixel to change the brightness
  // Stored brightness value is different than what's passed.
  // This simplifies the actual scaling math later, allowing a fast
  // 8x8-bit multiply and taking the MSB.  'brightness' is a uint8_t,
  // adding 1 here may (intentionally) roll over...so 0 = max brightness
  // (color values are interpreted literally; no scaling), 1 = min
  // brightness (off), 255 = just below max brightness.
  //12-3-13 LJB Added this new function to set the brightness of specific pixels
  //add NEO_PxB to the NeoPixel setup call...NeoPixel pixels = NeoPixel(NeoBright, PIN, NEO_GRB + NEO_KHZ800 + NEO_PxB);
  //call this function to change brightness of any pixel.  Still must call .show to update brightness
  uint8_t newBrightness = b + 1;
  if(newBrightness != bright[o] && o < numLEDs) { // Compare against prior value and make sure in bounds of bright array
    // Brightness has changed -- re-scale existing data in RAM
    uint8_t  c,
            *ptr           = pixels,
             oldBrightness = bright[o] - 1; // De-wrap old brightness value
    uint16_t scale;
	oldBrightness = bright[o] - 1; // De-wrap old brightness value
	if(oldBrightness == 0) scale = 0; // Avoid /0
	else if(b == 255) scale = 65535 / oldBrightness;
    ...
    for(uint16_t i = o * 3; i < o * 3 + 3; i++) {//move into the memory area of the pixel being changed only
		  c      = *ptr + i;
		  *ptr++ = (c * scale) >> 8;
	}
    bright[o] = newBrightness;
  }
}
